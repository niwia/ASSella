#!/usr/bin/env python3
"""
Vapor Manifest Engine (vapor.py)
--------------------------------
...
if not logger.handlers:
    ...
    handler.setFormatter(logging.Formatter("%(asctime)s [%(levelname)s] [Vapor] %(message)s"))
...
def generate_single_manifest(
        ...
        if mrc:
            ...
            if cdn_zip:
                ...
                if processed and len(processed) >= 4:
                    logger.info(
                        f"[Primary/Vapor] SUCCESS: Obtained {len(processed)} bytes manifest for "
    ...
    if HAS_MORRENUS and morrenus_api is not None:
        ...
        err_msg = "Primary Vapor CDN failed and Hubcap (morrenus_api) is not available"
...
def fetch_workshop_manifest(
    ...
    """
    Fetches a workshop item manifest using Vapor CDN primary with Hubcap smart fallback.
    ...
    Primary:
      ...
      2. If item is on SteamPipe (has hcontent_file) and depot key is known (from db/workshop_keys.txt),
         attempts Vapor generate_single_manifest(app_id, hcontent_file).
    ...
    # ── Path 1: Primary (Steam WebAPI + Vapor CDN) ──────────────────────────
    try:
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            # Record newly learned depot key to db/workshop_keys.txt for future Vapor CDN downloads
...
def fetch_manifests_by_build_id(
    ...
    each manifest through Vapor (with Hubcap smart fallback).
    """
    bid_str = str(build_id).strip()
...
def run_test():
    ...
    """
    print("=" * 65)
    print("  VAPOR MANIFEST ENGINE - TEST SUITE")
    print("=" * 65)
    ...
    if manifest_bytes and not err:
        ...
        import tempfile
        test_out_dir = Path(tempfile.gettempdir()) / "vapor_test_output"
    ...
    print("\n" + "=" * 65)
    print("  ALL TESTS PASSED! Vapor is ready for ASSella.")
    print("=" * 65)
    return True
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Vapor Manifest Engine for ASSella")
