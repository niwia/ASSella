...
class SingleDepotSelectionDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        self.subtitle_label = QLabel("Choose download destination and options")
        self.subtitle_label.setStyleSheet("font-size: 8.5pt; color: rgba(255, 255, 255, 0.6);")
    ...
    def _start_enrichment_async(self):
        ...
        current_depots = copy.deepcopy(self.depots)

        def _fetch():
            return get_depot_info_from_api(app_id_int, current_depots)

        def _on_done(enriched):
            if enriched and isinstance(enriched, dict):
                self._depots_enriched_signal.emit(enriched)

        self._enrich_runner = TaskRunner(self, "Enriching depot metadata", _fetch, finished_callback=_on_done)
        self._enrich_runner.start()
